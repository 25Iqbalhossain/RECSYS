...
/track events  ─────────────┐                               Catalog items
                            ▼                                      │
 data/raw_events/events.jsonl ──▶ Sessionize (30m gap)             │
                                   └─▶ data/sessions/sessions.parquet
                                          └─▶ Build Co‑vis pairs    │
                                               └─▶ data/artifacts/covis.parquet
                                                                   ▼
...
Client ▶ API ▶ (ANN candidates via FAISS) ▶ optional LTR reranker ▶ diversity (MMR) + freshness ▶ Response
                                 ▲
...
- ANN: `score` is cosine similarity (IP on L2‑normalized vectors), in [0, 1] typically.
...
- `semantic_score` is included on all results:
...
- FAISS index blank
  - Rebuild embeddings; ensure they’re valid (see `scripts/backfill_embeddings.py`), then rebuild FAISS; check build summary.
