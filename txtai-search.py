...
from txtai.pipeline import Similarity  # For semantic ranking

# LM Studio client (same as before)
...
def get_embedding(text, prefix="search_document: "):
    ...
    return response.data[0].embedding[:256]  # MRL to 256 dims

# LanceDB setup (same as before)
db_path = "./lancedb"
...
# Index documents using txtai for similarity pipeline, but store in LanceDB
documents = [
...
# Use txtai for semantic search (ranks results post-LanceDB query)
similarity = Similarity()  # Or load a multilingual model: Similarity("intfloat/multilingual-e5-base")

query_text = "লাল বৃত্ত "  # Or multilingual: "¿Qué es la búsqueda semántica?"
query_embedding = get_embedding(query_text, prefix="search_query: ")
results = table.search(query_embedding).limit(2).to_list()

# Refine with txtai similarity (optional, for additional ranking)
ranked = similarity(query_text, [r['text'] for r in results])
for idx, score in ranked:
    print(f"Score: {score:.4f} - Text: {results[idx]['text']}")